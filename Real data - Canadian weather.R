...
coef.mean = matrix(nrow = num.knots+3, ncol = length(canada.curve.list))
coef.qmedian = coef.mean
coef.q0.3 = coef.mean
coef.q0.7 = coef.mean

vec.mean = matrix(nrow = length(tc), ncol = length(canada.curve.list))
vec.q0.3 = vec.mean
vec.qmedian = vec.mean
vec.q0.7 = vec.mean
vec.original = vec.mean

for(j in 1:length(canada.curve.list)) vec.original[,j] = canada.curve.list[[j]]
...
for(j in 1:length(canada.curve.list)){
  ...
  vec.mean[,j] = basis_value_vector(coef.mean[,j], b.sp = b.sp.c)
  vec.qmedian[,j] = basis_value_vector(coef.qmedian[,j], b.sp = b.sp.c)
  vec.q0.3[,j] = basis_value_vector(coef.q0.3[,j], b.sp = b.sp.c)
  vec.q0.7[,j] = basis_value_vector(coef.q0.7[,j], b.sp = b.sp.c)
  
  bbbb = as.matrix(nearPD(t(b.basis.c)%*%b.basis.c)$mat)
  bmatrix[j,] = solve(bbbb, t(b.basis.c)%*%y)
}

basis = create.bspline.basis(rangeval = c(0, 1), num.knots+3)
argvals = matrix(rep(tc, length(canada.curve.list)), nrow = length(tc), ncol = length(canada.curve.list))
...
print(sum(RSQfunclust2$weights**2))

c2r <- RSQfunclust2$labels
print(c2r)
...
RSQfunclust2_k3 = RSKC.multidim(qsparse2, ncl = 3, l1bound = 3)
RSQfunclust1_k3 = RSKC(qsparse2, 3, alpha = 0.1, L1 = 3)

print("RSQfunclust1 weight")
print(RSQfunclust1_k3$weights)
print("RSQfunclust2 weight")
xx = sum((RSQfunclust2_k3$weights)**2)
yy = 1/sqrt(xx)
print(RSQfunclust2_k3$weights*yy)

c_result <- RSQfunclust2_k3$labels
location = CanadianWeather$coordinates
location[, 2] <- 360 - location[, 2]
poo1 = c_result == 1 #red
...
# c_input = as.vector(matrix(rep(c2tot, n_breaks), byrow = T, nrow = n_breaks))
# ID <- as.factor(ID)
# c_input <- as.factor(c_input)
# 
# x_grid = rep(seq(1, 12, 1), length(canada.curve.list))
# dataset = data.frame(ID, x_grid, c_input, vec0.3, vec.y)

# pdf("figure9-2.pdf")
# ggplot(data = dataset, mapping = aes(x = x_grid, y = vec0.3, group = ID, colour = c_input)) + 
#   geom_line(size = 0.5) +
...
# 
# pdf("figure9-1.pdf")
# ggplot(data = dataset, mapping = aes(x = x_grid, y = vec.y, group = ID, colour = c_input)) + 
#   geom_line(size = 0.5) +
...
c3r <- as.factor(RSQfunclust2_k3$labels)
x_grid2 = rep((1:100)*12/100, length(canada.curve.list))

dataset1.2 = data.frame(ID2, x_grid2, c_input2, vec0.3.2)
...
dataset1.1 = data.frame(ID, x_grid, c_input, vec.y)
...
# c_result <- kmeans(bmatrix, centers = 2, nstart = 10)$cluster
# print(c_result)
# 
# location = CanadianWeather$coordinates
# location[, 2] <- 360 - location[, 2]
