...
simulation2 <- function(sim, model, submodel, sig, curvenum = 100, simulnum = 200, num.knots = 10, n_breaks = 100){
  ...
  for(i in 1:simulnum){
    ...
    score.matrix.a = score.matrix[, c(1, 2, 3)]
    score.matrix.b = score.matrix[, c(1, 2, 3, 5)]
    score.matrix.c = score.matrix[, c(1, 2, 3, 4, 5, 6)]
    
    RSF.a <- RSKC(score.matrix.a, 2, alpha = 0.1, L1 = 3)
    RSF.b <- RSKC(score.matrix.b, 2, alpha = 0.1, L1 = 3)
    RSF.c <- RSKC(score.matrix.c, 2, alpha = 0.1, L1 = 3)
    ...
    print(paste("RSFclust2 (a)     : ", csr1.a[i], ari1.a[i], maf1.a[i]))
    ...
    print(paste("RSFclust2 (b)     : ", csr1.b[i], ari1.b[i], maf1.b[i]))
    ...
    print(paste("RSFclust2 (c)     : ", csr1.c[i], ari1.c[i], maf1.c[i]))
  ...
  ret = c(mean(csr1.a), mean(ari1.a), mean(maf1.a), mean(csr1.b), mean(ari1.b), mean(maf1.b), 
          mean(csr1.c), mean(ari1.c), mean(maf1.c), mean(csr2), mean(ari2), mean(maf2),
  ...
  ret = c(ret, sd(csr1.a), sd(ari1.a), sd(maf1.a), sd(csr1.b), sd(ari1.b), sd(maf1.b),
          sd(csr1.c), sd(ari1.c), sd(maf1.c), sd(csr2), sd(ari2), sd(maf2),
