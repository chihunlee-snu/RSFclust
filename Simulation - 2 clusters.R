...
simulation2 <- function(num, sc, errornum = 1, param = c(10, 0.5, 0, 0, 2.5, TRUE), curvenum = 50, simulnum = 50, pdiff = 0.2, sig = 0.2){
  csr1 = rep(0, simulnum)
  ari1 = rep(0, simulnum)
  maf1 = rep(0, simulnum)
  ...
  for(i in 1:simulnum){
    ...
    RSFclust2 = RSKC(qsparse, 2, alpha = 0.1, L1 = 3)
...
a = simulation2(1, 1, simulnum = 2, sig = 0.1) # 18 seconds per simulation
k.expectile2(a[[2]], a[[3]], 1, 1, 0.1) # save b-spline coefficient data, and upload it to the python code of K-expectie clustering


### Example 1

score2.1 = matrix(nrow = 8, ncol = 30)
...
for(i in 1:4){
  for(j in 1:2){
    ...
    a = simulation2(1, i, simulnum = 50, sig = sig)
    k.expectile2(a[[2]], a[[3]], 1, i, j)
    score2.1[(i-1)*2+j, ] = a[[1]]
  }
}

setwd("C://Users//user//Desktop//4학년 2학기//인턴//Code//simulation table") ### Your file directory
write.csv(score2.1, file = "score2.1.csv") ### Save the result file

# for(i in 1:9){
#   scorevec = simulation_score3[i,]
#   prnt = c("& CSR ")
#   for(j in 1:6){
#     prnt = c(prnt, paste0("& ", sprintf("%.1f", scorevec[(3*j-2)]), " (", sprintf("%.2f", scorevec[(3*j+16)]), ") "))
#   }
#   cat(prnt)
#   cat("\n")
#   prnt = c("& ARI ")
#   for(j in 1:6){
#     prnt = c(prnt, paste0("& ", sprintf("%.2f", scorevec[(3*j-1)]), " (", sprintf("%.2f", scorevec[(3*j+17)]), ") "))
#   }
#   cat(prnt)
#   cat("\n")
#   prnt = c("& MAF ")
#   for(j in 1:6){
#     prnt = c(prnt, paste0("& ", sprintf("%.1f", scorevec[(3*j)]), " (", sprintf("%.2f", scorevec[(3*j+18)]), ") "))
#   }
#   cat(prnt)
#   cat("\n")
# }
...
score2.2 = matrix(nrow = 12, ncol = 30)
...
for(i in 1:4){
 if(i != 3){ 
    for(j in 1:2){
      ...
      if(i < 3){
        a = simulation2(2, i, simulnum = 50, sig = sig)
        k.expectile2(a[[2]], a[[3]], 2, i, j)
        score2.2[(i-1)*2+j, ] = a[[1]]
      }
      else{
        a = simulation2(2, i, errornum = j, simulnum = 50)
        k.expectile2(a[[2]], a[[3]], 2, i, j)
        score2.2[10+j, ] = a[[1]]
      }
    }
   ...
   for(j in 1:6){
     sig = 0.1 + 0.2*(j%%2 == 0)
     a = simulation2(2, 3, errornum = 1+(j-1)%/%2, simulnum = 50, sig = sig)
     k.expectile2(a[[2]], a[[3]], 2, i, j)
     score2.2[4+j, ] = a[[1]]
   }
 }
...
write.csv(score2.2, file = "score2.2.csv")
