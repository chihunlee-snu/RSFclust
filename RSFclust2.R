...
FPC.score.matrix <- function(curve_list, quant.levels, expec.levels, K, 
  ...
  for(i in 1:q.num){
    ...
    if(i == 1) score.matrix = quant.score[[i]]
    else score.matrix = cbind(score.matrix, quant.score[[i]])
  }
  for(i in 1:e.num){
    ...
    if(!expec.none) score.matrix = cbind(score.matrix, expec.score[[i]])
  }
  return(score.matrix)
}
