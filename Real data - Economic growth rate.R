...
## 196개국, 44개 열 (1 + 43년) -> 이 중 2001~2022 (23:44) 이용
...
sum(rowSums(is.na(asia.gdp))==0) # 48개국중 29개
sum(rowSums(is.na(asia.gdp[,23:44]))==0) # 48개국중 44개
...
sum(rowSums(is.na(europe.gdp))==0) # 42개국 중 23개
sum(rowSums(is.na(europe.gdp[, 23:44]))==0) # 42개국 중 42개


eurasia_index <- gdp$region %in% eurasia.region
eurasia.gdp <- gdp[eurasia_index, ]
sum(rowSums(is.na(eurasia.gdp))==0) # 87개국 중 52개
sum(rowSums(is.na(eurasia.gdp[, 23:44]))==0) # 87개국 중 83개
...
# Afghanistan, Lebanon, Syria, North Korea -> cluster number 0으로
...
# Silhouette coefficient (클수록 good, K=3)
