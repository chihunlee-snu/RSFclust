...
quant.amcv <- function(curve, tau, n_breaks = 100, lambda = 0.1, num.knots = 10){
  b.basis = bsplineS(seq(from=0, to=1, length.out = n_breaks+1)[-(n_breaks+1)], seq(from=0, to=1, by=1/num.knots))
  D <- matrix(0, nrow=num.knots+1, ncol=num.knots+3)
  index <- c(1:(num.knots+1))
  D[cbind(index, index)] <- 1; D[cbind(index, index+1)] <- -2; D[cbind(index, index+2)] <- 1
  ...
  p = n_breaks%/%5
  miss_index = sample(1:n_breaks, p, replace = F)
  aocv = 0
  y = curve[-miss_index]
...
quant.amcv.select <- function(y, tau, lambda=exp(seq(-2, 4, length=7)), n_breaks=100, num.knots = 10, m=10){
  gcv.result <- rep(0, length=length(lambda))
  b.basis = bsplineS(seq(from=0, to=1, length.out = n_breaks+1)[-(n_breaks+1)], seq(from=0, to=1, by=1/num.knots))
  D <- matrix(0, nrow=num.knots+1, ncol=num.knots+3)
  index <- c(1:(num.knots+1))
  D[cbind(index, index)] <- 1; D[cbind(index, index+1)] <- -2; D[cbind(index, index+2)] <- 1
  for(i in 1:length(lambda)){
    ...
    for(j in 1:m){
      set.seed(j)
      gcv.result[i] <- gcv.result[i] + quant.amcv(y, tau, n_breaks, lamb, num.knots)
    }
    gcv.result[i] <- gcv.result[i]/m
  }
  #		print(gcv.result)
  return(lambda[which.min(gcv.result)])
}
quant.curve.pspline <- function(y, tau, n_breaks = 100, num.knots = 10, lambda = 0){
  if(lambda == 0) lambda <- quant.amcv.select(y, tau, n_breaks = n_breaks, num.knots = num.knots)
