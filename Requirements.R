...
curvelist_normal <- function(type, n_breaks = 100, N = 100, sd = 0.1){
  ...
  for(i in 1:N){
    if(type == 1) data = c(data, list(f1(t) + rnorm(length(t), 0, sd)))
...
curvelist_alaplace <- function(type, n_breaks = 100, N = 100, sigma = 0.1, alpha = 0.3, hetero = F, vert = 0){
  data = list()
  t = seq(0, 1, length.out = n_breaks+1)[1:n_breaks]
  if(!hetero){ # which means alpha is scalar
    for(i in 1:N){
      ...
      if(type == 1) newdata = f1(t) + ralaplace(length(t), mu, lam, kappa) + vert
      else if(type == 2) newdata = f2(t) + ralaplace(length(t), mu, lam, kappa) + vert
      else if(type == 3) newdata = f3(t) + ralaplace(length(t), mu, lam, kappa) + vert
    ...
    for(i in 1:N){
      ...
      for(j in 1:length(t)){
        ...
        mu = -lam*(1/kappa-kappa)/sqrt(2)
        if(type == 1) newdata = c(newdata, f1(k) + ralaplace(1, mu, lam, kappa) + vert)
        else if(type == 2) newdata = c(newdata, f2(k) + ralaplace(1, mu, lam, kappa) + vert)
        else if(type == 3) newdata = c(newdata, f3(k) + ralaplace(1, mu, lam, kappa) + vert)
      }
      data = c(data, list(newdata))
    }
  }
